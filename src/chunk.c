...
// Patch a 16-bit forward jump. Call after emitting the jump target.
// `offset` is the position of the high byte of the jump operand.
void patchJump(Chunk* chunk, int offset) {
    // The jump distance is from AFTER the jump instruction (3 bytes) to now.
    int jump = chunk->count - offset - 2;
    if (jump > 0xFFFF) {
        fprintf(stderr,
            NQ_COLOR_RED "error:" NQ_COLOR_RESET
            " Blok terlalu besar untuk di-jump.\n");
        exit(1);
    }
...
// ─────────────────────────────────────────────
//  Disassembler — human-readable bytecode dump
// ─────────────────────────────────────────────
...
int disassembleInstruction(Chunk* chunk, int offset) {
    ...
    switch (instruction) {
        ...
        default:
            printf("??? instruksi tidak dikenal: %d\n", instruction);
            return offset + 1;
    }
}
